...
<nav class="navbar navbar-inverse navbar-fixed-top">
 ...
 <br>
  ...
  $blogger = $_POST['blogger'];
   if(empty($_POST['blogger']) && empty($_POST['cat']))
  {
  ...
  {
    $result = mysql_query("SELECT Blog_Id,Blog_Title,Blog_Cat,Blog_Desc,User_Name,Image,Blog_Create_Date FROM blog natural join blogger WHERE (User_Name='$blogger' AND accept=1)") or die('Unable to run query:');
  }
  else if(empty($_POST['blogger']))
  {
  $result = mysql_query("SELECT Blog_Id,Blog_Title,Blog_Cat,Blog_Desc,User_Name,Image,Blog_Create_Date FROM blog natural join blogger WHERE( Blog_Cat='$cat' AND accept=1)") or die('Unable to run query:');
  }
  else
  { 
    $result = mysql_query("SELECT Blog_Id,Blog_Title,Blog_Cat,Blog_Desc,User_Name,Image,Blog_Create_Date FROM blog natural join blogger WHERE (Blog_Cat='$cat' AND User_Name='$blogger' AND accept=1)") or die('Unable to run query:');
  }
